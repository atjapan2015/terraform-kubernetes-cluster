resource "oci_core_instance" "k8s-node1" {
  ...
  metadata             = {
    ...
    user_data           = data.template_cloudinit_config.k8s-node1.rendered
  }
  preserve_boot_volume = false
  ...
  provisioner "local-exec" {
    command = "if [ $(kubectl --kubeconfig /u01/workspace/terraform/instances/k8s-node1/scripts/kubeconfig get nodes --no-headers=true | grep -v Ready | wc -l ) -gt 0 ]; then kubectl --kubeconfig /u01/workspace/terraform/instances/k8s-node1/scripts/kubeconfig get nodes --no-headers=true | grep -v Ready | awk '{print $1}' | xargs kubectl --kubeconfig /u01/workspace/terraform/instances/k8s-node1/scripts/kubeconfig drain --force;fi"
  }
