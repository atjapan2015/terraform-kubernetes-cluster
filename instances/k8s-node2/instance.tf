resource "oci_core_instance" "k8s-node2" {
  # Required
  ...
  metadata             = {
    ...
    user_data           = data.template_cloudinit_config.k8s-node2.rendered
  }
  preserve_boot_volume = false
...
output "k8s-node2-name" {
  value = oci_core_instance.k8s-node2.display_name
}
output "k8s-node2-public-ip" {
  value = oci_core_instance.k8s-node2.public_ip
}
output "k8s-node2-ocid" {
  value = oci_core_instance.k8s-node2.id
}